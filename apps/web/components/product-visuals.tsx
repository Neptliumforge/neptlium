...
export function CapitalSystemVisual() {
  return (
    <div className="capital-system-visual" aria-label="Four connected Neptlium product systems">
      ...
      <div>
        {['Portfolio Intelligence', 'Capital Account', 'Treasury', 'Allocation'].map(
          (item, index) => (
            <article key={item}>
              <span>0{index + 1}</span>
              <h3>{item}</h3>
...
export function OperatingModelVisual() {
  const stages = [
    ...
    ['Organize', 'Structure capital deliberately.'],
    ['Authorize', 'Govern action explicitly.'],
    ['Settle', 'Resolve canonical state.'],
  ] as const;
  return (
...
export function PlatformArchitectureVisual() {
  const domains = ['Portfolio', 'Treasury', 'Allocation'] as const;
  ...
  return (
    ...
    >
      <div className="platform-architecture-title">
        <span>Neptlium</span>
        <strong>Capital position</strong>
        <small>Observed and classified</small>
      </div>
      <span className="platform-architecture-connector" aria-hidden="true" />
      <div className="platform-architecture-domains">
      ...
      <span className="platform-architecture-connector" aria-hidden="true" />
      <div className="platform-architecture-account">
        <span>Operating boundary</span>
        <strong>Capital Account</strong>
      </div>
      <span className="platform-architecture-connector" aria-hidden="true" />
      <div className="platform-architecture-provider">
        <span>Provider / network infrastructure</span>
        <small>External evidence · reconciliation required</small>
      </div>
    </div>
  );
}
...
export function PortfolioVisual() {
  return (
    <ProductFrame title="Portfolio Intelligence" state="Illustrative structure">
      ...
      <div className="composition-band" aria-hidden="true">
        <i />
        <i />
        <i />
      </div>
  ...
  return (
    <ProductFrame title="Capital Account" state="Eligibility applies">
      ...
      <p className="visual-disclosure">
        Funding becomes available only when account provisioning and eligibility permit it.
      </p>
    </ProductFrame>
  );
}
      ...
      <div className="treasury-tree">
        ...
        <ul>
          <li>
            ...
            <i className="liquidity-a" />
            <ProductState>Available</ProductState>
          </li>
          <li>
            <span>Reserve</span>
            <i className="liquidity-b" />
            <ProductState>Reserved</ProductState>
          </li>
          <li>
            <span>Committed capital</span>
            <i className="liquidity-c" />
            <ProductState>Defined</ProductState>
          </li>
        </ul>
      </div>
  ...
  return (
    <ProductFrame title="Allocation" state="Modeled">
      <div className="allocation-compare">
        <figure>
          <figcaption>Current position</figcaption>
          <div aria-hidden="true">
            {allocationRoles.map((role, i) => (
              <i key={role} style={{ flex: i + 2 }} />
            ))}
          </div>
        ...
        <figure>
          <figcaption>Modeled position</figcaption>
          <div aria-hidden="true">
            {allocationRoles.map((role, i) => (
              <i key={role} style={{ flex: 6 - i }} />
            ))}
          </div>
...
export function SecurityFlowVisual() {
  ...
  return (
    <ol className="security-flow" aria-label="Neptlium security architecture">
      {boundaries.map((item, i) => (
        <li key={item}>
          <span>0{i + 1}</span>
          <strong>{item}</strong>
          {i < boundaries.length - 1 && <i aria-hidden="true" />}
        </li>
      ))}
    </ol>
  );
