  ...
  return (
    <section className="cin-story" aria-labelledby="operating-story-title">
      <div className="cin-story-stage" data-stage={active}>
        ...
        <p className="cin-caption">Illustrative Neptlium operating environment</p>
      </div>
      <div className="cin-story-copy">
        <header className="cin-story-intro">
          <p className="cin-kicker">The operating model</p>
          <h2 id="operating-story-title">One environment. Distinct states of meaning.</h2>
        </header>
        {stages.map((stage, index) => (
          <article
            className="cin-story-step"
            data-stage={index}
            ref={(node) => { refs.current[index] = node; }}
            key={stage.kicker}
          >
            <p className="cin-kicker">{stage.kicker}</p>
            <h3>{stage.title}</h3>
            <p>{stage.body}</p>
          </article>
        ))}
        <div className="cin-story-resolution">
          <span>Capital state</span><i>→</i><span>Context</span><i>→</i><span>Governance</span><i>→</i><span>Consequence</span>
        </div>
      </div>
    </section>
  );
