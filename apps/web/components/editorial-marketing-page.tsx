...
export function EditorialMarketingPage({eyebrow,title,lead,chapters,cta}:{eyebrow:string;title:string;lead:string;chapters:readonly {title:string;body:string}[];cta?:{label:string;href:string}}){return <div className="editorial-page">
  <section className="editorial-hero n-rail-route-hero"><CapitalRails variant="evidence" decorative className="n-rail-route-hero__field" /><p>{eyebrow}</p><h1>{title}</h1><span>{lead}</span>{cta?<Link className="cin-pill cin-pill-light" href={cta.href}>{cta.label}<ArrowRight aria-hidden="true"/></Link>:null}</section>
