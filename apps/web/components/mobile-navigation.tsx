    ...
    <div id="mobile-command-sheet" className="mobile-command-sheet" ref={panelRef}>
      <div className="mobile-command-head">
        ...
        <div className="mobile-command-head-actions">
          <Link className="mobile-command-entry" href={SITE.signUpUrl}>Sign up</Link>
      ...
      <nav className="mobile-command-nav" aria-label="Mobile navigation">
        <Link className="mobile-platform-link" href={platform.href} aria-current={path === platform.href ? 'page' : undefined}><span>{platform.label}</span><ArrowRight aria-hidden="true" /></Link>
        <div className="mobile-nav-grid">{sections.map((section) => <section key={section.label}>
          <Link className="mobile-section-label" href={section.href}>{section.label}</Link>
          <div className="mobile-section-links">{section.links.map((link) => <Link href={link.href} key={link.href} aria-current={path === link.href ? 'page' : undefined}>{link.label}</Link>)}</div>
        </section>)}</div>
