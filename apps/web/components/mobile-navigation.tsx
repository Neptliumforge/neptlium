...
export function MobileNavigation({ path, onClose, triggerRef }: { path: string; onClose: () => void; triggerRef: RefObject<HTMLButtonElement | null>; }) {
  ...
  return <div className="mobile-command-wrap" role="dialog" aria-modal="true" aria-label="Navigation">
    <div id="mobile-command-sheet" className="mobile-command-sheet" ref={panelRef}>
      <div className="mobile-command-head"><Brand tone="teal" /><div className="mobile-command-head-actions"><Link className="mobile-command-entry" href={SITE.signInUrl}>Sign in</Link><button ref={closeRef} type="button" aria-label="Close navigation" onClick={onClose}><X aria-hidden="true" /></button></div></div>
      <nav className="mobile-command-nav" aria-label="Mobile navigation">
        <div className="mobile-nav-grid">{NAVIGATION.map((section) => <section key={section.label}><Link className="mobile-section-label" href={section.href} aria-current={path === section.href ? 'page' : undefined}><span>{section.label}</span><ArrowRight aria-hidden="true" /></Link>{section.links.length > 1 ? <div className="mobile-section-links">{section.links.filter((link) => link.href !== section.href).map((link) => <Link href={link.href} key={link.href}>{link.label}</Link>)}</div> : null}</section>)}</div>
        <Link className="mobile-enter-action" href={SITE.signUpUrl}>Open account <ArrowRight aria-hidden="true" /></Link>
