...
function fakeRepository(role = 'super_admin') {
  const auditEntries = [];
  return {
    auditEntries,
    getRole: async () => role,
    getSession: async (id) => ({ id, email: null, fullName: null, displayName: null, role: 'super_admin' }),
    getDashboard: async () => ({}),
...
test('Supabase authentication alone does not imply General Platform Administrator authorization', async () => {
  const response = await executeAdminHttp(config, { ...base, method: 'GET', url: '/v1/admin/session' }, { repository: fakeRepository('admin'), authenticate });
...
test('exact super_admin may access privileged admin API', async () => {
  const response = await executeAdminHttp(config, { ...base, method: 'GET', url: '/v1/admin/session' }, { repository: fakeRepository('super_admin'), authenticate });
...
test('unauthenticated caller cannot reach privileged admin API', async () => {
  const response = await executeAdminHttp(config, { method: 'GET', url: '/v1/admin/session', headers: {}, clientAddress: '127.0.0.1' }, { repository: fakeRepository(), authenticate });
  assert.equal(response.statusCode, 401);
});

test('withdrawal approval remains fail closed and cannot mark settlement', async () => {
