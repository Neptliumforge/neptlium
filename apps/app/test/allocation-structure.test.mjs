...
test('Allocation customer surface is API-owned and contains no provider SDK authority', () => {
  ...
  for (const provider of [
    ...
    assert.equal(
      combined.toLowerCase().includes(provider.toLowerCase()),
      false,
      `Allocation customer code imports or names provider SDK ${provider}`,
    );
  }
  assert.equal(
    /\.from\s*\(|\.rpc\s*\(/.test(combined),
    false,
    'Allocation customer code must not access Supabase data directly',
  );
});
...
test('Allocation review language separates authorization from execution', () => {
  ...
  assert.equal(workspace.includes('Authorization records an approved decision'), true);
  assert.equal(workspace.includes('Execution unavailable'), true);
  for (const forbidden of ['Invest now', 'Auto rebalance', '>Buy<', '>Sell<', 'Optimize portfolio'])
    assert.equal(workspace.includes(forbidden), false);
});
