...
export function TreasuryView({
  ...
  return (
    <Stack>
      <WorkspaceHeader
        eyebrow="Liquidity and movement"
        title="Treasury"
      ...
      <section className="grid gap-6 border-b border-border-hairline pb-6 lg:grid-cols-[minmax(0,1.4fr)_repeat(3,minmax(8rem,0.6fr))] lg:items-end">
        ...
        <div><p className="text-xs text-text-muted">Withdrawals</p><p className="mt-1 text-sm font-medium text-text-primary">{enabledTransfers.length ? 'Available' : 'Unavailable'}</p></div>
      </section>
      ...
      <Section title="Deposit readiness">
        ...
        <p className="mt-3 text-xs text-text-muted">{enabledFunding.length ? `${enabledFunding.length} deposit route${enabledFunding.length === 1 ? '' : 's'} available.` : 'No deposit routes are available for this account yet.'}</p>
      </Section>
      ...
      <Section title="Treasury controls">
        <div className="divide-y divide-border-hairline border-y border-border-hairline">
          {[
            ['Balance integrity', 'Balances are shown only when an account position has been established.'],
            ['Destination verification', 'Saving a destination does not mean it is verified or active.'],
            ['Withdrawal review', 'Reservation and approval remain separate from submission.'],
