...
export function AllocationWorkspace({ workspace }: { readonly workspace: Workspace }) {
  ...
  return (
    <Stack>
      ...
      <ol className="flex flex-wrap gap-x-5 gap-y-2 border-y border-border-hairline py-3" aria-label="Allocation lifecycle">
        {(['Observed','Modeled','Authorized','Executed','Reconciled'] as const).map((label, index) => (
          <li key={label} className="flex items-center gap-2 text-xs">
            <span className="tabular-nums text-accent-primary">0{index + 1}</span>
      ...
      <Section title="Current allocation">
        <p className="mb-3 text-xs text-text-muted">Canonical source · {workspace.observed.source} · as of {new Date(workspace.observed.asOf).toLocaleString()}</p>
        <div className="border-y border-border-hairline">
          {workspace.observed.positions.length === 0 ? (
            <ProductStateMessage state="NO_ACTIVITY" title="No canonical positions yet">Allocation observes only posted Neptlium ledger positions.</ProductStateMessage>
            ...
            return (
              <div key={`${position.asset}:${position.network ?? ''}`} className="grid gap-2 border-b border-border-hairline py-4 last:border-0 sm:grid-cols-[minmax(8rem,1fr)_minmax(10rem,auto)_auto] sm:items-center sm:gap-5">
      ...
      <Section title="Policy">
        <div className="border-y border-border-hairline py-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><p className="text-sm font-medium text-text-primary">{policy ? `${policy.name} · v${policy.version}` : 'Define allocation policy'}</p><p className="mt-1 text-xs text-text-muted">Draft changes are not authorization.</p></div>
            {policy ? <ProductStateBadge state={policy.status === 'AUTHORIZED' ? 'AVAILABLE' : 'PENDING'}>{policy.status === 'AUTHORIZED' ? 'Authorized' : 'Draft'}</ProductStateBadge> : null}
          </div>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <div><Label htmlFor="allocation-review">Review frequency</Label><select id="allocation-review" className="mt-2 h-11 w-full rounded-md border border-border-default bg-surface-1 px-3 text-sm text-text-primary" value={draft.reviewFrequency} onChange={(event) => setDraft((value) => ({ ...value, reviewFrequency: event.target.value }))}><option value="MANUAL">Manual</option><option value="WEEKLY">Weekly</option><option value="MONTHLY">Monthly</option><option value="QUARTERLY">Quarterly</option><option value="ANNUALLY">Annually</option></select></div>
          ...
          <div className="mt-5 border-t border-border-hairline pt-4">
            ...
            <p className="mt-2 text-xs text-text-muted">Targets must total 100%. Classification is policy input, not an investment recommendation.</p>
          </div>
      ...
      <Section title="Review">
        <div className="border-y border-border-hairline py-5">
          {!latestPlan ? <ProductStateMessage state="NO_ACTIVITY" title="No modeled plan yet">Review rebalance creates a persistent model and plan. It does not move capital.</ProductStateMessage> : <>
            ...
            <div className="mt-4 border-t border-border-hairline pt-4"><ProductStateBadge state="UNAVAILABLE">Execution unavailable</ProductStateBadge><p className="mt-2 max-w-2xl text-sm leading-6 text-text-muted">This allocation can be modeled and authorized. Capital movement will remain unavailable until the required execution capability is active.</p></div>
