...
export async function saveAllocationPolicyAction(input: { policyId?: string; expectedVersion?: number; policy: AllocationPolicyInput }): Promise<AllocationMutationResult> {
  try {
    const result = input.policyId && input.expectedVersion
      ? await updateAllocationPolicy(input.policyId, input.expectedVersion, input.policy)
      : await createAllocationPolicy(input.policy);
...
export async function authorizeAllocationPolicyAction(policyId: string, expectedVersion: number): Promise<AllocationMutationResult> {
  try { const result = await authorizeAllocationPolicy(policyId, expectedVersion); return { ok: true, message: `Policy v${result.policy.version} authorized.`, id: result.policy.id }; }
  catch (error) { return { ok: false, error: errorMessage(error) }; }
}

export async function modelAllocationAction(policyId: string): Promise<AllocationMutationResult> {
  try { const result = await createAllocationModel(policyId); return { ok: true, message: 'Allocation model created from canonical capital.', id: result.model.id }; }
  catch (error) { return { ok: false, error: errorMessage(error) }; }
}

export async function reviewRebalanceAction(policyId: string): Promise<AllocationMutationResult> {
  try {
    const model = await createAllocationModel(policyId);
    const plan = await createAllocationPlan(model.model.id);
...
export async function authorizeAllocationPlanAction(planId: string): Promise<AllocationMutationResult> {
  try { const result = await authorizeAllocationPlan(planId); return { ok: true, message: result.execution === 'UNAVAILABLE' ? 'Plan authorized. Execution remains unavailable.' : 'Plan authorized.', id: result.plan.id }; }
  catch (error) { return { ok: false, error: errorMessage(error) }; }
}
