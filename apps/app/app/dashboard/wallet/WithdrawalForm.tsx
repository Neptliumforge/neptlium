...
export function WithdrawalForm({ pairs }: WithdrawalFormProps) {
  ...
  if (pairs.length === 0) {
    return (
      <p className="text-body-sm text-text-secondary">
        No supported withdrawal assets are configured yet.
      </p>
    );
  }
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4">
      ...
      <Field>
        <Label htmlFor="withdrawal-destination">Verified withdrawal address</Label>
        <Input
          id="withdrawal-destination"
          name="destination"
          placeholder="Select or paste a verified destination"
        />
      ...
      <p className="rounded-md border border-warning/30 bg-warning/10 p-3 text-xs leading-5 text-text-secondary">
        Withdrawals enter pending review and are not completed until authorized operations or
        provider confirmation finalizes execution.
      </p>
      {state?.ok && (
        <p className="text-body-sm text-accent-primary">Withdrawal request submitted for review.</p>
      )}
