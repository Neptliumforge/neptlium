...
export function createAllocationPolicy(input: AllocationPolicyInput) { return apiRequest<{ policy: AllocationPolicy; replayed: boolean }>('/v1/allocation/policies', { method: 'POST', body: JSON.stringify(input) }); }
export function updateAllocationPolicy(policyId: string, expectedVersion: number, policy: AllocationPolicyInput) { return apiRequest<{ policy: AllocationPolicy; replayed: boolean }>(`/v1/allocation/policies/${encodeURIComponent(policyId)}`, { method: 'PATCH', body: JSON.stringify({ expectedVersion, policy }) }); }
export function authorizeAllocationPolicy(policyId: string, expectedVersion: number) { return apiRequest<{ policy: AllocationPolicy; replayed: boolean }>(`/v1/allocation/policies/${encodeURIComponent(policyId)}/authorize`, { method: 'POST', body: JSON.stringify({ expectedVersion }) }); }
export function createAllocationModel(policyId: string) { return apiRequest<{ model: { id: string }; replayed: boolean }>('/v1/allocation/models', { method: 'POST', body: JSON.stringify({ policyId }) }); }
export function createAllocationPlan(modelId: string) { return apiRequest<{ plan: AllocationPlan; replayed: boolean }>('/v1/allocation/plans', { method: 'POST', body: JSON.stringify({ modelId }) }); }
export function authorizeAllocationPlan(planId: string) { return apiRequest<{ plan: AllocationPlan; replayed: boolean; execution: 'UNAVAILABLE' }>(`/v1/allocation/plans/${encodeURIComponent(planId)}/authorize`, { method: 'POST', body: JSON.stringify({}) }); }
export function cancelAllocationPlan(planId: string, reason: string) { return apiRequest<{ plan: AllocationPlan; replayed: boolean }>(`/v1/allocation/plans/${encodeURIComponent(planId)}/cancel`, { method: 'POST', body: JSON.stringify({ reason }) }); }
