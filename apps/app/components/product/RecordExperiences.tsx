...
export function CompaniesExperience() {
  return <div className="op-stack">
    ...
    <section className="op-panel">
      <Heading label="Investment context" title="Companies" />
      <div className="op-evidence-canvas">
        <div className="op-evidence-grid" aria-hidden="true" />
        <div className="op-evidence-message"><Building2 size={18}/><strong>No authenticated company exposure projection is available</strong><p>Neptlium will not infer portfolio exposure from public market research. Company rows will appear here when investment relationships and position context are provided by an authoritative account projection.</p></div>
      </div>
    </section>
...
export function DocumentsExperience() {
  ...
  return <div className="op-stack">
    <Header eyebrow="Records" title="Documents" description="Statements, reports and account evidence associated with your governed capital state." />
    <section className="op-panel"><Heading label="Document record" title="Available files"/>
      {documents === null ? <div className="op-empty-row"><strong>Documents unavailable</strong><span>The document projection could not be loaded. No missing file is represented as an empty account.</span></div> : documents.length === 0 ? <div className="op-empty-row"><strong>No documents yet</strong><span>Statements, reports and account files will appear when they are recorded.</span></div> : <div className="op-document-list">{documents.map((document) => <div className="op-document-row" key={document.id}><FileText size={16}/><div><strong>{document.title}</strong><span>{document.category.replaceAll('_',' ')} · {new Date(document.createdAt).toLocaleDateString()}</span></div><DownloadButton documentId={document.id}/></div>)}</div>}
    </section>
  </div>;
}
...
export function NotificationsExperience() {
  ...
  return <div className="op-stack">
    <Header eyebrow="Records" title="Notifications" description="Account, security and financial-lifecycle notices that require awareness or action." action={unread > 0 ? <MarkAllReadButton/> : undefined}/>
    <section className="op-panel"><Heading label="Inbox" title={notifications ? `${unread} unread` : 'Notification state'}/>
      {notifications === null ? <div className="op-empty-row"><strong>Notifications unavailable</strong><span>The notification projection could not be loaded.</span></div> : notifications.length === 0 ? <div className="op-empty-row"><strong>No notifications</strong><span>There are no account notices recorded for this snapshot.</span></div> : <ul className="op-notification-list">{notifications.map((item) => <NotificationItem key={item.id} {...item}/>)}</ul>}
    </section>
  </div>;
}
...
export function SettingsExperience() {
  ...
  return <div className="op-stack">
    <Header eyebrow="Account" title="Settings" description="Identity, organization and security context for this Neptlium account." />
    <section className="op-grid-2">
      <article className="op-panel"><Heading label="Profile" title="Account identity"/><div className="op-settings-list">
        <div><UserRound size={16}/><span><small>Name</small><strong>{account.fullName ?? account.displayName ?? 'Not provided'}</strong></span></div>
        <div><Bell size={16}/><span><small>Email</small><strong>{account.email ?? 'Not provided'}</strong></span></div>
        <div><ShieldCheck size={16}/><span><small>Compliance</small><strong>{account.complianceStatus ?? 'Not reported'}</strong></span></div>
      </div></article>
      <article className="op-panel"><Heading label="Organization" title="Operating context"/>{organization ? <div className="op-settings-list"><div><Building2 size={16}/><span><small>Organization</small><strong>{organization.name ?? 'Unnamed organization'}</strong></span></div><div><ShieldCheck size={16}/><span><small>Role</small><strong>{organization.role ?? account.role}</strong></span></div><div><Building2 size={16}/><span><small>Industry</small><strong>{organization.industry ?? 'Not provided'}</strong></span></div></div> : <div className="op-empty-row"><strong>No organization context</strong><span>This account does not currently expose an organization projection.</span></div>}</article>
    </section>
