    ...
    <div className="relative">
      ...
      {open && (
        <div ref={panel} role="dialog" aria-modal="true" aria-label="Profile menu" className="fixed inset-x-0 bottom-0 z-50 max-h-[92dvh] overflow-y-auto rounded-t-xl border border-border-default bg-surface-1 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-lg sm:absolute sm:inset-auto sm:right-0 sm:top-[calc(100%+8px)] sm:w-80 sm:rounded-lg sm:p-3">
          ...
          <div className="border-b border-border-hairline px-2 pb-3 pr-12 sm:pr-2"><p className="truncate text-sm font-medium">{name || 'Account'}</p><p className="truncate text-xs text-text-muted">{email || 'Email not provided'}</p>{verified && <p className="mt-1 text-xs text-success">Verified identity</p>}<p className="mt-1 text-xs text-text-muted">Personal account</p></div>
          ...
          <Link href="/dashboard/settings#treasury-upgrade" onClick={() => setOpen(false)} className="mx-2 my-2 block rounded-lg border border-border-default bg-surface-2 p-3"><span className="block text-xs font-medium text-accent-primary">NEPTLIUM TREASURY</span><strong className="mt-1 block text-sm">Upgrade to Treasury access →</strong><span className="mt-1 block text-xs text-text-muted">Create a separate business workspace without changing your personal Capital account.</span></Link>
