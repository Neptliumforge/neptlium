...
export default async function WithdrawalsPage({
  ...
  return (
    <div>
      <PageHeader
        title="Withdrawals"
        description="Review governed movement approvals without implying submission or settlement"
      />
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <ConfirmDialog
                          ...
                          description={`Approve a movement of ${formatAmount(row.amount)} for ${row.user_email ?? 'this user'}? Approval does not submit to a provider or confirm settlement.`}
                          confirmLabel="Approve"
                          onConfirm={async () => {
                            await approveWithdrawal(row.id);
                          }}
                        />
                        <span className="text-[11px] text-text-muted">Rejection unavailable</span>
                      </div>
                    </td>
                  </tr>
                ))}
