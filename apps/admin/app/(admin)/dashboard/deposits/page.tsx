...
export default async function DepositsPage({
  ...
  const { rows, total } = await getDeposits({
    page,
  ...
  return (
    <div>
      <PageHeader
        title="Deposits"
        description={`${total} funding records — manual completion remains unavailable until governed reconciliation is connected`}
      />
                    ...
                    <td className="px-4 py-3">
                      {(row.status === 'pending' || row.status === 'pending_review') && (
                        <span className="text-[11px] text-text-muted">
                          Awaiting governed reconciliation
                        </span>
                      )}
                    </td>
                  </tr>
