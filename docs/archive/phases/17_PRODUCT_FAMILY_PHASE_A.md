...
| Neptlium Treasury | `apps/treasury` | `treasury.neptlium.com` | Authenticated organization treasury product |
...
`app.neptlium.com` and `treasury.neptlium.com` are separate products. Neptlium Treasury does not replace Neptlium Capital.
...
## Neptlium Treasury organization boundary

Neptlium Treasury models authority conceptually as:
...
Neptlium Treasury treats a treasury account as a business/accounting and authority concept. Wallets remain technical execution infrastructure.
...
The Neptlium Treasury payment shell may display the governed lifecycle vocabulary from draft through reconciliation, but Phase A exposes no direct browser execution.
...
`apps/pay` is intentionally isolated from the authenticated Neptlium Treasury shell. Its public route is `/i/[publicToken]`; the opaque public token is a presentation identifier, not settlement authority. Sequential invoice IDs are not the security boundary.
...
Public service categories are limited to customer-meaningful groups: Website, Capital, Neptlium Treasury, API, Payments, Provider Ingress, Notifications, and Documents.

## Admin alignment

Phase A does not add empty Admin screens. Future operational support will be required for organization identity/membership, Neptlium Treasury payment intents, approvals, policy evidence, reconciliation, provider events and public incidents. Those surfaces should be added only with real backend projections and authorization.
...
Each app owns its domain/project root while sharing monorepo packages and the canonical API boundary. No standalone Neptlium Treasury repository is created.
