...
const directory = new URL('supabase/functions/crypto-webhook/', root);
...
async function retired(response) {
  ...
  assert.equal(body.error, 'CRYPTO_WEBHOOK_RETIRED');
  assert.match(body.message, /retired.*governed provider ingress/i);
...
test('malformed, sample and fake provider payloads with absent or invalid auth remain retired', async () => {
  const handler = load();
  for (const body of [
    '{',
    '',
    '{"name":"synthetic-test"}',
    '{"type":"gate02.synthetic.invalid","id":"not-a-provider-event"}',
  ])
...
test('the sole executable source cannot import clients or access provider, database, network or secrets', () => {
  ...
  assert.doesNotMatch(
    executable,
    /provider_webhook_inbox|provider_references|settlement_evidence|transfer_execution|funding_intents|transactions|portfolios|ledger_|reconciliation_|SUPABASE_|SECRET|https?:/i,
  );
...
test('configuration pins the guard and retains JWT verification', () => {
  ...
  const section = config.match(/\[functions\.crypto-webhook\]([^[]*)/);
  assert.ok(section);
  assert.match(section[1], /verify_jwt\s*=\s*true/);
  assert.match(section[1], /entrypoint\s*=\s*"\.\/functions\/crypto-webhook\/index\.ts"/);
});

test('application, shared packages and CI cannot call or deploy the legacy webhook slug', () => {
  try {
    const matches = execFileSync(
      ...
      [
        ...
        '-i',
        '-E',
        'crypto[-_]webhook|cryptowebhook',
        '--',
        'apps',
        'packages',
        '.github',
    ...
    assert.fail(`Legacy webhook reference: ${matches}`);
  } catch (error) {
