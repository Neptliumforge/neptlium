...
create unique index platform_owners_individual_unique
  on public.platform_owners(individual_user_id)
  where individual_user_id is not null;
create unique index platform_owners_organization_unique
  on public.platform_owners(organization_id)
...
create unique index canonical_assets_network_symbol_contract_unique
  on public.canonical_assets(network_identifier, symbol, contract_address) nulls not distinct;

-- Bridge the existing canonical ledger into the owner registry without changing historical semantics.
-- New posting paths can populate platform_owner_id while existing rows remain valid until backfill is reviewed.
alter table public.ledger_accounts
  add column if not exists platform_owner_id uuid references public.platform_owners(id) on delete restrict;

alter table public.ledger_journals
  add column if not exists platform_owner_id uuid references public.platform_owners(id) on delete restrict;

create index if not exists ledger_accounts_platform_owner_idx
  on public.ledger_accounts(platform_owner_id, asset, network);
create index if not exists ledger_journals_platform_owner_idx
  on public.ledger_journals(platform_owner_id, posted_at desc);
...
create index platform_audit_events_owner_created_idx
  on public.platform_audit_events(platform_owner_id, created_at desc);
create unique index platform_audit_events_idempotency_unique
  on public.platform_audit_events(platform_owner_id, event_type, idempotency_key)
...
revoke all on public.platform_owners, public.organization_memberships, public.canonical_assets, public.platform_audit_events
  from public, anon, authenticated;

-- Service code may read the control plane. Mutation remains explicit and server-side.
grant select on public.platform_owners, public.organization_memberships, public.canonical_assets, public.platform_audit_events
  to service_role;
grant insert, update on public.platform_owners, public.organization_memberships, public.canonical_assets
  to service_role;
...
create trigger platform_audit_events_append_only
  ...
  for each row execute function public.reject_platform_audit_event_mutation();

-- Guard immutable subject identity. Status can change, subject ownership cannot.
...
create trigger platform_owners_identity_guard
  before update on public.platform_owners
  for each row execute function public.enforce_platform_owner_identity();
