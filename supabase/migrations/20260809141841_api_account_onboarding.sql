-- Complete authenticated onboarding atomically behind the API service role.
-- The browser must never receive service-role credentials or call this RPC.
...
begin
  ...
  if organization_name is not null and organization_id is null then
  ...
  update public.profiles set
    ...
    organization_id = complete_account_onboarding.organization_id,
  ...
  values (target_user_id, 'user') on conflict do nothing;
  ...
  if not exists (select 1 from public.wallets where profile_id = target_user_id) then
    insert into public.wallets (portfolio_id, profile_id, provider)
    values (portfolio_id, target_user_id, 'internal');
  end if;

  delete from public.onboarding_drafts where user_id = target_user_id;
  return query select target_user_id;
end;
$$;

revoke all on function public.complete_account_onboarding(uuid, jsonb) from public, anon, authenticated;
grant execute on function public.complete_account_onboarding(uuid, jsonb) to service_role;
