-- Clerk-only identity cutover.
--
-- Runtime authentication is Clerk-only. Existing principal UUIDs are preserved by
-- linking a server-verified Clerk subject to the unique active profile whose
-- normalized email matches Clerk's verified primary email. Legacy provider rows
-- remain inert historical evidence and are never accepted as runtime credentials.
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended('clerk-email:' || v_email, 0));
  ...
  if v_existing_email_count = 1 then
    ...
    insert into public.identity_provider_subjects (
      ...
      v_existing_email_principal, 'CLERK', p_clerk_subject
    ...
    insert into public.identity_events (
      ...
      v_existing_email_principal,
      v_subject_id,
      'provider_subject.linked',
      nullif(btrim(p_request_id), ''),
      'clerk-only-cutover:subject:' || p_clerk_subject,
      jsonb_build_object(
        'provider', 'CLERK',
        'source', 'clerk_verified_email_cutover',
