...
create table public.treasury_destination_events (
  ...
  actor_id uuid references public.identity_principals(id) on delete restrict,
...
create table public.treasury_destination_challenges (
  ...
  created_by uuid not null references public.identity_principals(id) on delete restrict,
...
-- Clerk cutover already owns public.assert_treasury_super_admin(uuid).
-- Do not restore the superseded Supabase-auth implementation here.
